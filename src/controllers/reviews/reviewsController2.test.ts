...
import { getbyBrand, getbyBrandbyOwner, getReviews } from "./reviewsController";
...
describe("Given a get reviews  controller", () => {
  const req = {
    query: {} as unknown,
  };
  const res = {
...
describe("Given a get by brand and owner controller", () => {
  const req = {
    params: { brand: "nike", owner: "123456799" } as unknown,
  };
  const res = {
    status: jest.fn().mockReturnThis(),
    json: jest.fn(),
